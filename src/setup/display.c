...
#ifdef HAVE_LIBPNG
      txt_new_check_box("Save screenshots in PNG format", &png_screenshots),
#endif
      txt_new_check_box("Smooth pixel scaling", &smooth_pixel_scaling), NULL);
...
void ConfigDisplay(TXT_UNCAST_ARG(widget), void *user_data)
{
  ...
  txt_signal_connect(advanced_button, "pressed", AdvancedDisplayConfig,
                     sizes_table);
}
