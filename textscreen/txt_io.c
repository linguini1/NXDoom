...
void TXT_PutSymbol(int c) { PutSymbol(TXT_GetScreenData(), c); }
...
void TXT_PutChar(int c) { PutChar(TXT_GetScreenData(), c); }
...
void TXT_FGColor(txt_color_t color) { fgcolor = color; }

void TXT_BGColor(int color, int blinking)
{
  bgcolor = color;
  if (blinking) bgcolor |= TXT_COLOR_BLINKING;
}
