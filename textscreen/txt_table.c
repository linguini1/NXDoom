...
static void CellOverflowedSize(txt_table_t *table, int x, int y, int *w,
                               int *h)
{
  txt_widget_t *widget;
  int x1, y1;
...
static void CalculateWidgetDimensions(txt_table_t *table, int x, int y,
...
static void CalcRowColSizes(txt_table_t *table, unsigned int *row_heights,
      ...
      for (x = 0; x < table->columns; ++x)
        {
          ...
          CalculateWidgetDimensions(table, x, y, col_widths, row_heights, &w,
                                    &h);
          if (w < widget->w)
            {
...
void TXT_AddWidget(TXT_UNCAST_ARG(table), TXT_UNCAST_ARG(widget))
{
  ...
  is_separator =
      IsActualWidget(widget) && widget->widget_class == &txt_separator_class;
  ...
  if (i >= 0 && i < table->num_widgets)
    {
      ...
      return IsActualWidget(widget) && TXT_SelectableWidget(widget) &&
             widget->visible;
    }
...
static void LayoutCell(txt_table_t *table, int x, int y, int draw_x,
                       int draw_y)
{
  txt_widget_t *widget;
  int col_width;
      ...
      for (x = 0; x < table->columns; ++x)
        {
          ...
          if (IsActualWidget(widget))
            {
              CalculateWidgetDimensions(table, x, y, column_widths,
                                        row_heights, &widget->w, &widget->h);
...
txt_widget_class_t txt_table_class = {
    TXT_TableSelectable, TXT_CalcTableSize,   TXT_TableDrawer,
    TXT_TableKeyPress,   TXT_TableDestructor, TXT_TableMousePress,
    TXT_TableLayout,     TXT_TableFocused,
};
